...
# Bot configuration
GUILD_ID = None  # Will be set from environment variable
...
# New data storage
bot_config = {
    'command_permissions': {},  # command_name: [role_ids]
    'admin_roles': [],
    'moderator_roles': []
}
...
def has_permission(user_roles, command_name):
    ...
    if command_name not in bot_config['command_permissions']:
...
def guild_only():
    """Decorator to restrict commands to the configured guild"""
...
class ConfigView(discord.ui.View):
    ...
    async def view_config(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        guild_name = "Not restricted"
        if GUILD_ID:
            guild = bot.get_guild(GUILD_ID)
            guild_name = guild.name if guild else f"Guild ID: {GUILD_ID}"
        embed.add_field(name="Guild Restriction", value=guild_name, inline=False)
        ...
        all_commands = [
            "config", "autoresponder", "autoresponders", "auctionsetup", "auctioncreate", 
            "embedcreator", "reactionroles", "boostsetup", "invitesetup", "invites", 
            "connect4", "endgame", "test_autoresponder", "export_autoresponders"
        ]
        
        if bot_config['command_permissions']:
            perms_text = ""
            for cmd in all_commands:
                ...
                if roles:
                    ...
                    perms_text += f"**{cmd}:** All roles\n"
            embed.add_field(name="Command Permissions", value=perms_text, inline=False)
        else:
            embed.add_field(name="Command Permissions", value="All commands available to all roles", inline=False)
...
class CommandPermissionsModal(discord.ui.Modal):
    ...
    command = discord.ui.TextInput(
        label="Command Name",
        placeholder="Available: config, autoresponder, autoresponders, auctionsetup, auctioncreate, embedcreator, reactionroles, boostsetup, invitesetup, invites, connect4, endgame, test_autoresponder, export_autoresponders",
        max_length=50
    )
    
    roles = discord.ui.TextInput(
        label="Allowed Role IDs (comma-separated)",
        placeholder="Enter role IDs or leave empty for no restrictions",
        required=False,
        max_length=500
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"Permissions updated for command: {self.command.value}", ephemeral=True)
...
class GuildSettingsModal(discord.ui.Modal):
    ...
    guild_id = discord.ui.TextInput(
        ...
        placeholder="Enter guild ID to restrict bot to one server, or leave empty for no restriction",
        required=False,
        max_length=20
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not self.guild_id.value:
            GUILD_ID = None
            await interaction.response.send_message("Guild restriction removed. Bot can be used in any server.", ephemeral=True)
            return
        
        try:
            guild_id = int(self.guild_id.value)
            guild = bot.get_guild(guild_id)
            
            if not guild:
                await interaction.response.send_message("Guild not found. Make sure the bot is in that server and the ID is correct.", ephemeral=True)
                return
            
            GUILD_ID = guild_id
            await interaction.response.send_message(f"Bot restricted to guild: {guild.name} (ID: {guild_id})", ephemeral=True)
...
async def config_command(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "config"):
...
async def embed_creator_command(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "embedcreator"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def reaction_roles_command(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "reactionroles"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def boost_setup_command(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "boostsetup"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def invite_setup_command(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "invitesetup"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def autoresponder_command(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "autoresponder"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def autoresponders_command(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "autoresponders"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def auction_setup(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "auctionsetup"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def auction_create(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "auctioncreate"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
async def export_autoresponders(interaction: discord.Interaction):
    if not has_permission(interaction.user.roles, "export_autoresponders"):
        await interaction.response.send_message("You don't have permission to use this command.", ephemeral=True)
        return
...
# Set guild restriction if provided
if guild_id_env:
    try:
        GUILD_ID = int(guild_id_env)
        print(f"Bot restricted to guild ID: {GUILD_ID}")
    except ValueError:
        print("Invalid DISCORD_GUILD_ID format. Bot will run without guild restriction.")
        GUILD_ID = None
else:
    GUILD_ID = None
...
if __name__ == "__main__":
    if not token:
        ...
        if GUILD_ID:
            ...
            print("Bot will work in all servers")
